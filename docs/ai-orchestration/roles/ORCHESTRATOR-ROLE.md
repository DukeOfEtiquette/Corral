...
6. Apply any task-specific STATUS deltas to the hand-authored intent sections if they changed (Current phase, Next step where present, Blocked on). The activity surface (`last_updated`, `recent_updates`) is derived from git per ADR-039; no manual STATUS edit for the activity history. The roadmap and next-step are derived; a resolved task keeps its `epic:` linkage so the ETL rolls it up as done.
...
- **Name task-specific STATUS deltas (rule R6).** The activity surface (`last_updated`, `recent_updates`) is derived from git per ADR-039 and is never hand-edited. `status_deltas` names task-specific edits to the hand-authored STATUS sections (Current phase, Next step where present, Blocked on), or the literal `"none"` when there are none. Write the literal `"none"` rather than omitting the section, so the absence is a checked signal rather than an ambiguous gap.
