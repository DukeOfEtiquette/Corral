...
3. Dispatch **`worker-agent`** (`model: sonnet`, foreground) to implement until all tests pass. The worker may not create or edit test files (the no-touch rule in `WORKER-ROLE.md`). If the worker believes a test is wrong, it returns `RETURN: ESCALATION`; route the correction to a FRESH `test-designer` dispatch (back to phase 1), never to a worker edit of the test.
...
If the implementation worker returns `RETURN: ESCALATION` asserting a test is wrong, the Orchestrator routes the fix to a FRESH `test-designer` dispatch (a new phase 1 run for the affected test), never to a worker edit. After the corrected tests are authored, a new phase 2 implementation dispatch runs against them.
