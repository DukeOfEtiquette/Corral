...
6. Apply STATUS hygiene plus the task-specific deltas: bump `last_updated` and prepend a `recent_updates` entry. The roadmap and next-step are now derived (no manual STATUS edit required); a resolved task keeps its `epic:` linkage so the ETL rolls it up as done.
...
- **Name task-specific STATUS deltas (rule R6).** The Executor handles universal STATUS hygiene (bump `last_updated`, append a `recent_updates` entry in `./ai-infrastructure/project-manager/STATUS.md`) per `EXECUTOR-ROLE.md`. The kickoff names any task-specific STATUS edits beyond that (phase changes, "Blocked on" updates). If there are none, write the literal "universal hygiene only" rather than omitting the section, so the absence is a checked signal rather than an ambiguous gap.
