...
- **Audience: the Worker agent.** A kickoff is read by the dispatched `worker-agent` subagent (Sonnet, role-loaded, per `./decisions/ADR-028-worker-as-dispatched-subagent.md`). Write the doc as instructions to the Worker, not to the user. No "How to invoke" sections, no "Open a fresh session and run..." meta-content (rule R7); the kickoff is a brief, and the brief's reader is the agent that executes it.
...
- **Omit universal conventions** already covered by `WORKER-ROLE.md` and the `worker-agent`: the writing rules and Agent Discipline in `./CLAUDE.md`, the compose-only run policy, git boundaries, the pinned six-section report shape. Reference these; do not re-emit them.
