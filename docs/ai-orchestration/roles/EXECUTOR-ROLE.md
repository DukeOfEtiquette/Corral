...
This document defines the Executor role for Corral, right-sized from the rogue exemplar per `./ai-infrastructure/project-manager/decisions/ADR-009-adopt-rogue-orchestration-conventions.md`. The Executor carries out a self-contained kickoff prompt against a tight plan, reports back in a pinned shape, and returns. "Execute" means carry out the kickoff's deliverables, whatever their kind (docs, specs, configs, agent definitions, code) -- not strictly code. It is the execution counterpart to the Orchestrator role defined in `./docs/ai-orchestration/roles/ORCHESTRATOR-ROLE.md`.

The role is adopted by the `executor` subagent, which the Orchestrator dispatches via the Task tool (`./ai-infrastructure/project-manager/decisions/ADR-028-worker-as-dispatched-subagent.md`). This is the single executor execution path in Corral; ADR-028 retired the former `/corral-worker` slash command. The dispatch-specific deltas (the Executor returns to the Orchestrator rather than the user, escalates by return value, and runs no checker subagents) are named in "Identity (dispatched subagent)" below; everything else in this document applies unchanged.

## Identity (dispatched subagent)

The Executor role is adopted by the dispatched `executor` (ADR-028). Three deltas distinguish the dispatched executor from a free-standing session; they override the matching prose elsewhere in this document:

- **You return to the Orchestrator, not the user.** Your final message IS the return value the Orchestrator consumes. It begins with a verdict line, `RETURN: COMPLETED` or `RETURN: ESCALATION`, so the Orchestrator can branch without parsing prose. The full return schemas live in `./.claude/agents/specs/EXECUTOR-AGENT-SPEC.md`.
- **You escalate by return value, not by asking.** Where this document says "surface to the user" or "ask the user" (the failure modes, the ambiguity rules), you instead return `RETURN: ESCALATION` with the four-part block. The Orchestrator answers simple cases and re-dispatches a fresh executor; edge cases it surfaces to the user.
...
Explicit context pass-down is the rule: you read exactly the `explicit_reads` the Orchestrator names (plus the kickoff and, on re-dispatch, the resume anchor), in order. You do not survey state or deduce your workspace. The agent file `./.claude/agents/executor.md` and its spec carry the full input package and workflow phases.

## Scope

The Executor operates on a single kickoff prompt in a single dispatch. Its job is mechanical execution against a tight, well-specified plan: read the kickoff, read the files the kickoff names, make the changes the kickoff specifies, return a structured report.

It is not a runtime agent in the autonomous sense (no scheduled invocations, no self-launched follow-up sessions); it is the user-facing session the human engineer opens to carry out the work the Orchestrator already planned. The Executor does not survey repo state, does not pattern-mine, does not propose new work. Direction comes from the kickoff; deviations get raised to the user, not absorbed.

## Responsibilities

The role comprises four activity clusters. An Executor session typically touches all of them in order.
...
- Stay within the files and directories the kickoff names. Edits outside that scope require user confirmation, not Executor discretion.
...
These apply to every Executor session. Per-task content lives in the kickoff; the boundary table below captures the split.
...
These are the situations an Executor encounters most often where the right move is to stop, not to push through. As a dispatched subagent the Executor stops by returning `RETURN: ESCALATION` to the Orchestrator (per "Identity (dispatched subagent)"); read every "ask the user" / "surface to the user" below as "escalate to the Orchestrator."

- **Ambiguous kickoff.** The kickoff names a deliverable but does not specify the format, file path, or acceptance criterion. Ask the user; do not guess. The Orchestrator may have left the choice for the user, not the Executor.
...
- **Out-of-scope discoveries.** While executing the kickoff, the Executor finds an adjacent issue (a typo, a stale comment, a bug). Note it under "Follow-ups" in the report; do not fold it into the current changeset.
...
If a prior Executor session crashed mid-kickoff, a fresh Executor session can resume by reading:
...
**Re-dispatch (after an escalation) uses the same pattern.** When the Orchestrator re-dispatches a fresh executor following a `RETURN: ESCALATION` (ADR-028), the resume sources are the kickoff, the prior partial report (`resume_anchor`), and the `prior_progress_summary`, with the Orchestrator's `escalation_answer` treated as a pinned decision. Do not re-execute the work the prior attempt already completed; continue from the resume point.

## Report shape

End every Executor session with these six sections, in this order. Sections may be empty (record "(none)" rather than omitting the heading) so the Orchestrator's review pass can scan a consistent shape.
...
(Where the kickoff left a choice and the Executor resolved it. State the choice and the rationale. If a decision was deferred to the user mid-session, record that too.)
...
The pinned shape is the contract between Executor and Orchestrator. An Executor that returns prose without these sections has not completed the role's responsibilities, even if the underlying work is correct.

### Dual-channel: print to chat AND write to file

Every Executor session writes the six-section report to two channels:
...
The activity surface (`last_updated`, `recent_updates`) is derived from git per ADR-039 and is never hand-edited. The Executor edits the workspace STATUS file only when the kickoff's `status_deltas` field names a task-specific edit to one of the hand-authored STATUS sections (Current phase, Next step where present, Blocked on).

The target is the STATUS of the workspace the task belongs to: for a coordinator task that is `./ai-infrastructure/project-manager/STATUS.md`; for a department task (ADR-031, each department owns its own task tree) it is that department's `STATUS.md` (for example `./ai-infrastructure/database/STATUS.md`). The kickoff's STATUS-deltas section names the exact file.
...
The `executor` runs on Sonnet: its agent file pins `model: sonnet`, and the Orchestrator dispatches it with the `model: sonnet` override (ADR-028). The convention follows from the principle "Opus decides and plans, Sonnet executes": Executor work is mechanical execution against a tight plan, which benefits from Sonnet's speed and cost profile. The dispatch override pins the executor's tier independent of the Orchestrator's, so the Opus-plans / Sonnet-executes split holds even though both run within one user-facing session.
...
## Executor / kickoff-prompt boundary

| Lives in `EXECUTOR-ROLE.md` (universal) | Lives in `executor.md` + spec (the agent) | Lives in kickoff prompt (per task) |
|---|---|---|
| Executor role identity and scope | Bootstrap reads and workflow phases | Specific artifact in scope, and its domain (ADR-005) |
...
The Orchestrator's kickoffs reference this role doc (and, where useful, the `executor`) rather than re-emitting their content. Universal conventions stay in one place.

## Not in scope

- **Surveying repo state.** Executors do not run state surveys, read STATUS files (except when `status_deltas` names a hand-authored section edit to apply), scan ADRs, enumerate scratch artifacts, or list tasks. That is the Orchestrator's job. The kickoff carries forward whatever survey context the Executor needs, including its "Related tasks and ADRs" section.
- **Drafting new kickoffs.** Executors consume kickoffs; they do not produce them. If execution surfaces work that warrants a separate kickoff, it goes under "Follow-ups" in the report, not into a new artifact authored by the Executor.
- **Running the Orchestrator command.** The Executor does not invoke the Orchestrator command (any `/<slug>-orchestrator`) to "load context"; that loads survey state and a conflicting role identity. The Executor's required reads are exactly the `explicit_reads` the Orchestrator names plus the kickoff.
- **Pattern-mining and observation logging.** Patterns surfaced during execution go under "Follow-ups" so the Orchestrator can decide whether to log them. The Executor does not write to `./ai-infrastructure/project-manager/OBSERVATIONS.md` or propose ADRs.
- **Task transitions.** The Executor never moves, edits, or creates files under `./ai-infrastructure/project-manager/tasks/`; see "Universal conventions".

## Checker dispatch (Orchestrator-run)

Per `./ai-infrastructure/project-manager/decisions/ADR-023-dispatch-loop-day-zero.md`, two cross-department checker subagents gate every executor run. Because the dispatched `executor` is a leaf (a dispatched subagent has no Agent/Task tool, ADR-028), the **Orchestrator** runs both checkers around the executor; the Executor dispatches neither. The full protocol is canonical in `ORCHESTRATOR-ROLE.md`, section "Dispatched-worker flow" (steps 2 and 5); this section names the two checkpoints and what they enforce so the Executor knows the contract its kickoff and report are held to. If departments are created later (ADR-021), department-scoped checkers may layer beside the cross-department pair, mirroring rogue's universal-vs-workspace-scoped split; until then the cross-department pair is the whole surface.

Two checkpoints, both Orchestrator-run:

1. **Prelaunch (before the executor is dispatched).** The Orchestrator dispatches `worker-prelaunch-checker` with the kickoff path. It enforces rule W1: every deferral the kickoff carries must name an acceptance test or a user-confirm flag. On FAIL the Orchestrator does not dispatch the executor; it routes the kickoff back through the drafter+checker loop or surfaces to the user. Spec: `./.claude/agents/specs/WORKER-PRELAUNCH-CHECKER-SPEC.md`.
2. **Close (after the executor returns COMPLETED).** The Orchestrator dispatches `worker-close-checker` with the report path the executor wrote. It enforces rule W2: every Follow-ups item must name a target phase, a "COR-T candidate" tag, or a triage flag. On FAIL the Orchestrator surfaces a three-exit menu (accept-with-rationale / manually-edit / re-dispatch a corrective executor), per the Dispatched-worker flow step 5. Spec: `./.claude/agents/specs/WORKER-CLOSE-CHECKER-SPEC.md`. On an implementation close (where the Orchestrator supplies `protected_test_paths`), rule W3 conditionally joins W2: FAIL if any protected test path appears in the report's "Files touched" section. W3 is inert when `protected_test_paths` is empty (including on test-design closes and tasks with no protected tests).

**What this means for the Executor.** The Executor writes a kickoff-faithful report (the six sections, dual-channel) and returns; it does not run, wait on, or branch on the checkers. Anchoring its Follow-ups items (W2) and not silently absorbing deferrals (W1) are still the Executor's obligations, because the Orchestrator's checkers will catch violations; the Executor simply is not the one dispatching them.

## Instantiation

The role is adopted by the `executor` subagent (`./.claude/agents/executor.md`), which the Orchestrator dispatches via the Task tool (ADR-028). On dispatch the agent:

1. Runs on Sonnet (its agent file pins `model: sonnet`; the Orchestrator dispatches with the `model: sonnet` override).
2. Reads its bootstrap pair (`./.claude/agents/specs/EXECUTOR-AGENT-SPEC.md` and this document) so it adopts the Executor role with the Identity deltas. Role name: "Executor Agent".
...
The full dispatch package, workflow phases, and return schemas live in `./.claude/agents/specs/EXECUTOR-AGENT-SPEC.md`.
