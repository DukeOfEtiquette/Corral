...
- 2026-06-29: Resolved by the Backend API Orchestrator and moved to done. All three items shipped and independently re-derived on disk. Item 1: single tracked `app/.env.example` at the compose project dir; the compose `api` service now drives `DATABASE_URL`, `API_DOCS_ENABLED`, and `SESSION_*` from `app/.env` via `${VAR:-default}` (defaults match `app/api/settings.py`); scattered `app/api/.env.example` and `app/db/.env.example` deleted (`docker compose config` resolves all five to documented defaults; only the `api` service block changed). Item 3: `gen-admin-hash.sh` header gains the reseed gotcha (idempotent `seed_admin`; `down -v` to re-seed) and an `app/.env.example` pointer; no smoke test (decided); no behavior change. Item 2: two missing-admin-creds fail-fast regression tests in `test_admin_seed.py` (`api-test` compose suite: 30 passed). All checker gates PASS (2 kickoff-checker, 2 prelaunch W1, 2 close W2). Deliverables + the four kickoff/report handoff artifacts (ADR-024) + triaged follow-ups DB-T-006 (db-dept DATABASE_URL unification) and API-T-009 (.dockerignore cleanup) committed in 7843f81; this done-move follows in the next commit.
