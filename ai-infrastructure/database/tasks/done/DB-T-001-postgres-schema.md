...
- 2026-06-11: Resolved. ADR-014 accepted (Alembic, hand-written migrations, no ORM) to unblock; decisions pinned (full auth schema in one baseline; machine users via a separate agent_credentials table). Kickoff drafted+checked (2 iterations), prelaunch PASS, worker-agent dispatched, close PASS; deliverable independently re-verified against a Postgres compose service. app/db/ Alembic baseline migration 0001 (eleven tables) + app/docker-compose.yml committed as 74de6b2. Moved to tasks/done/. status -> done.
