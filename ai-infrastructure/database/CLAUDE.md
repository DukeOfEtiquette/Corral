# Corral - AI infrastructure: Database department workspace

This file is the operating rules for AI agents working inside the `ai-infrastructure/database/` department workspace. It supplements the global rules in the repo-root `CLAUDE.md` (which remains authoritative for Agent Discipline, writing style, secrets, documentation placement, and the two-domains framing). When a rule here conflicts with the global file, surface it to the user; do not silently resolve it.
