...
All three ADR-027 follow-on backbone tasks are now complete: COR-T-012 (ai-infrastructure restructure), COR-T-013 (create-department recipe), and COR-T-014 (project-manager dashboard). The dashboard reads the markdown sources (source: "markdown") and will repoint to the Corral app at the dogfood milestone (ADR-008). COR-T-008 (ADR-018: label taxonomy) is now resolved. Remaining Phase 1 work is the two pending-ADR resolution tasks: COR-T-009 (resolve ADR-025: native epics) and COR-T-010 (resolve ADR-026: per-agent MCP identity).
