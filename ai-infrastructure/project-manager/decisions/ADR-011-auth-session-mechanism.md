...
The MCP server holds a long random bearer secret in its gitignored `.env` (per ADR-006 secret convention) and presents it as a bearer token to the REST API. One shared service identity for all agents. Per-agent identity is deferred to a future pending ADR. **Forward pointer (COR-T-053, 2026-06-24):** that "future pending ADR" is ADR-026 (accepted 2026-06-10); see `./ADR-026-per-agent-mcp-identity.md` and Consequence #6.
