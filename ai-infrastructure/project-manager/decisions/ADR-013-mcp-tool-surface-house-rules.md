...
5. **ADR-025 epics deferral; dropped `issue_link` rationale.** Native epics (parent-child issue grouping) are deferred to ADR-025 (pending), which will amend ADR-012's schema via a new ADR (following the ADR-024 precedent: an accepted ADR is amended by a later ADR, not edited in place) and add epic tools additively to this surface per ADR-019 policy. `issue_link` is deliberately not ported because Corral will support epics natively rather than via the body-convention workaround.
