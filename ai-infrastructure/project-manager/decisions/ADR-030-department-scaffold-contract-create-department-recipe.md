...
> **Forward pointer (2026-06-14):** ADR-041 adds a forming-epic step to the recipe: a department is stamped with at least one forming epic for its active or next phase at creation time, so its anticipated work is represented in the derived roadmap (otherwise a phase reads done when a member department's epic is simply unfiled). This refines the ADR-037 pointer above: the `epics/` tree is no longer left entirely to first-use lazy creation; the recipe stamps the tree and a forming epic, since a department's reason for existing is itself that forming epic. The forming epic may carry zero tasks (`planned`) per ADR-036. Implemented by COR-T-051. See ADR-041.
