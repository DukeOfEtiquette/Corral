...
8. **ADR-038 phase model (forward pointer).** Roadmap Phases are modeled as Views plus a reserved `phase:*` label (ADR-038), orthogonal to the epic/task `parent_id` chain this ADR defines: phase membership rides a label on epics, not the parent relation, so this ADR's flat one-level model is unchanged. See `./ADR-038-phase-as-first-class-view.md`.
