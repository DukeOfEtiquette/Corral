...
> **Forward pointer (2026-06-14):** kickoff rule **R6** ("STATUS-deltas section present") is retired by ADR-040 / COR-T-050. ADR-040 made the STATUS current-state surface fully derived and removed the hand-authored `## Current phase` / `## Next step` / `## Blocked on` body sections, so the `status_deltas` field R6 enforced has no target. R6 is tombstoned in the kickoff-checker spec (it emits no finding); the other rule numbers (R1-R5, R7, R8, and the worker rules W1-W3) are unchanged. The rule-lineage table below is left as the day-zero historical record. See ADR-040 and `KICKOFF-CHECKER-SPEC.md` Phase 5.
