...
> **Forward pointer (2026-06-11):** ADR-031 reverses Fork B below. The "Single shared `dept:`-labeled task pool" decision (and its alternatives section, and the `tasks/` line in the Decision tree) is superseded: every workspace, the coordinator and each department, owns its own `tasks/` tree with its own ID prefix (rogue-faithful), and the `dept:<slug>` label is applied at the dogfood import (ADR-008) rather than hand-applied to partition a shared pool now. Forks A, C, D (as amended by ADR-030), and E are otherwise unaffected. See ADR-031.
...
> **Forward pointer (2026-06-13):** ADR-039 structurally closes a drift gap the coordinator write authority granted below enabled: cross-workspace writes (a coordinator task creating artifacts inside a department, e.g. COR-T-044 creating `database/epics/`) used to leave the target workspace's hand-maintained STATUS activity fields stale. ADR-039 derives those fields (`last_updated`, `recent_updates`) from `git log -- <workspace-path>`, so a cross-workspace write surfaces in the target's feed automatically and there is no hand-maintained field to forget. See ADR-039.
