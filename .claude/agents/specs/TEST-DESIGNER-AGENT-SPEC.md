...
1. Apply STATUS hygiene ONCE per `TEST-DESIGNER-ROLE.md` section "Wrap-up STATUS hygiene": bump `last_updated`, append one `recent_updates` entry, apply the kickoff's named `status_deltas`. This is the ONLY phase that mutates the workspace STATUS file.
2. Write the full six-section report to `report_path` (dual-channel). List `report_path` and the workspace STATUS file under "Files touched".
...
Side effects before returning: the identical six sections are written to `report_path`; STATUS hygiene is applied once. "Files touched" lists `report_path` and the workspace STATUS file.
...
5. **STATUS-once.** Mutate the workspace STATUS file only on COMPLETED, exactly one `recent_updates` entry and one `last_updated` bump.
...
**Return:** `RETURN: COMPLETED` + the six-section report; `API-T-001-TEST-DESIGN-KICKOFF-REPORT.md` and the department STATUS file written. All files touched are test files.
