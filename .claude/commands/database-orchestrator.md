...
- `ai-infrastructure/database/STATUS.md` holds only hand-authored forward intent (`Current phase` / `Next step` / `Blocked on`); update those when intent changes. The activity history (`last_updated`, `recent_updates`) is derived from git per ADR-039 and is not hand-maintained.
